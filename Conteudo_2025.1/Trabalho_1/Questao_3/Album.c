...
void rotacionaEsq_album(Arvore_Album **raiz) {
    Arvore_Album *temp = (*raiz)->dir;
    (*raiz)->dir = temp->esq;
    temp->esq = *raiz;
    *raiz = temp;
}

void rotacionaDir_album(Arvore_Album **raiz) {
    Arvore_Album *temp = (*raiz)->esq;
    (*raiz)->esq = temp->dir;
    temp->dir = *raiz;
    *raiz = temp;
}

int maior_altura_album(int a, int b) {
    return (a > b) ? a : b;
}

int pegaAltura_album(Arvore_Album *raiz) {
    int altura;
    if(raiz != NULL){
        altura = 1 + maior_altura_album(pegaAltura_album(raiz->esq), pegaAltura_album(raiz->dir));
    }else{
        altura = 0;
    }
    return altura;
...
void atualiza_altura_album(Arvore_Album *raiz){
    if(raiz != NULL){
        raiz->altura = maior_altura_album(pegaAltura_album(raiz->esq), pegaAltura_album(raiz->dir)) + 1;
    }
}

void balanceamento_album(Arvore_Album **raiz) {
    int fator_balanceamento = pegaAltura_album((*raiz)->esq) - pegaAltura_album((*raiz)->dir);
    if(fator_balanceamento > 1) {
        if(pegaAltura_album((*raiz)->esq->esq) >= pegaAltura_album((*raiz)->esq->dir)) {
            rotacionaDir_album(raiz);
        } else {
            rotacionaEsq_album(&(*raiz)->esq);
            rotacionaDir_album(raiz);
        }
    } else if(fator_balanceamento < -1) {
        if(pegaAltura_album((*raiz)->dir->dir) >= pegaAltura_album((*raiz)->dir->esq)) {
            rotacionaEsq_album(raiz);
        } else {
            rotacionaDir_album(&(*raiz)->dir);
            rotacionaEsq_album(raiz);
        }
    }
}
